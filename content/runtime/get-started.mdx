...
import { Tab, Tabs } from "@components/nextra-theme";
...
Let's say you want to run version `3.12` of the Wasmer package [`python/python`](https://wasmer.io/python/python) and execute the `-c "print(1+1)"` argument to print a result.
...
<Tabs items={['CLI', 'JS' /*, 'Python'*/]}>
  ...
  <Tab>
    ...
    await init();
    let python = await Wasmer.fromRegistry("python/python@3.12");
