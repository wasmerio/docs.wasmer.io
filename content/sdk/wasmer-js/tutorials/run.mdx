import { Card, Callout, Steps } from "@components/nextra-theme";
...
which is crucial for running our Markdown renderer.
...
fn main() -> std::io::Result<()> {
    ...
    pulldown_cmark::html::write_html_io(std::io::stdout(), parser)?;
    Ok(())
}
```
...
STDOUT with `pulldown_cmark::html::write_html_io()`.
...
target/wasm32-wasip1/release/markdown-renderer.wasm: WebAssembly (wasm) binary module version 0x1 (MVP)
```
...
`./markdown-renderer/target/wasm32-wasip1/release/markdown-renderer.wasm` file.
...
```ts filename="index.ts" copy
async function renderMarkdown(command: Command, markdown: string) {
  const instance = await command.run({ stdin: markdown });
...
dev tools, you'll probably see a message about `SharedArrayBuffer`:
    ...
    plugins: [
        {
            ...
            buildStart: () => {
                return new Promise((resolve, reject) => {
                    exec(
                        "cargo build --target=wasm32-wasip1 --manifest-path=markdown-renderer/Cargo.toml --release --quiet",
                        (err, stdout, stderr) => {
                            if (err) {
