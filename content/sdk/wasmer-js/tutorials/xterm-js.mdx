import { Cards, Card, Callout, Steps } from '@components/nextra-theme';
...
Once initialized, install `@wasmer/sdk`, `@xterm/xterm`, and `@xterm/addon-fit`
by running:

```sh copy
npm install @wasmer/sdk @xterm/xterm @xterm/addon-fit
```

These packages are crucial; `@wasmer/sdk` is our WebAssembly runtime, while
`@xterm/xterm` and its add-on are used to create the terminal interface in the browser.
...
You may have noticed that we're using a dynamic import (`await import("@wasmer/sdk")`) to pull in `@wasmer/sdk` rather than a "normal" `import { ... } from "@wasmer/sdk"`.
...
[coi]: /sdk/wasmer-js/explainers/troubleshooting#sharedarraybuffer-and-cross-origin-isolation
[document-undefined]: /sdk/wasmer-js/explainers/troubleshooting#referenceerror-document-is-not-defined
