...
import { Card } from '@components/nextra-theme';
...
This will install the `wasm32-wasip1` target for the Rust compiler.
...
As you see above in the source property, in the `module` section. We use the release build in wasm32-wasip1 target.

```bash copy
cargo build --release --target wasm32-wasip1
```

This will create a `hello-cgi.wasm` file in the `target/wasm32-wasip1/release` directory.
...
[[module]]
name = "server"
source = "target/wasm32-wasip1/release/hello-cgi.wasm"
abi = "wasi"
...
<Callout type="info" emoji="ℹ️">
  The local `wasmer run` command can only execute WASI(X) packages, so a WCGI
  package cannot be run locally. WCGI packages still deploy and run on Wasmer
  Edge, so we test the app by deploying it.
</Callout>
...
<Callout type="info" emoji="ℹ">
    The deployment URL follows the format `https://<app-name>.wasmer.app`. If
    the name is already taken, a suffixed variant such as
    `https://<app-name>-<app-owner>.wasmer.app` is used instead.
</Callout>
...
We can also send a `POST` request with a name in the body:

```bash copy
curl -X POST -d "Wasmer" https://hello-cgi-wasmer-user.wasmer.app
```
