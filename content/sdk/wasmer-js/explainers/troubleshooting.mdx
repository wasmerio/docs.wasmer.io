...
import { Callout, Steps } from '@components/nextra-theme';
...
GitHub Pages Deployment*](/sdk/wasmer-js/how-to/coop-coep-headers) for more.
...
```typescript
import { init, runWasix, Wasmer } from "@wasmer/sdk";
...
using `WebAssembly.compile()` and `runWasix()` directly, the browser doesn't provide
...
let instance = await runWasix(ffmpeg, {
    program: "ffmpeg",
    args: ["-i", "input.mp4", "-vf", `"transpose=2"`, "output.mp4"],
});
let result = await instance.wait();
...
This error typically arises when executing the browser build of `@wasmer/sdk`
within a Node.js environment.

### Root Cause

The default `@wasmer/sdk` entry point targets browsers and relies on
browser-specific features, such as `navigator.hardwareConcurrency`, which
Node.js does not provide.

### Solution

Node.js is supported through the dedicated `@wasmer/sdk/node` entry point.
Import the SDK from there when running under Node.js:
...
```typescript
// Before
import { Wasmer, init, initializeLogger, Instance } from "@wasmer/sdk/wasm-inlined";

// After
const { Wasmer, init, initializeLogger } = await import("@wasmer/sdk/wasm-inlined");
```
