...
The template ships its CDN configuration in the `settings/config.toml` file,
which configures the [`wasmer/static-web-server`](https://wasmer.io/wasmer/static-web-server)
package that serves your files. When you deploy, the files in `./public` are
bundled together with that configuration and served by the web server.

If you add your own `wasmer.toml` manifest to the project, this is the
underlying package wiring it declares:
