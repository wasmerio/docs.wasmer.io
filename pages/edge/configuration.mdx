...
See the dedicated [SSH/SFTP documentation](/edge/learn/ssh) for more details.
...
* Usernames must be prefixed with the app short ID: `<app-shortid>_myusername`
  ...
  The short id is can be found in the `Unique URL` field: `https://<short-id>.id.wasmer.app`.
