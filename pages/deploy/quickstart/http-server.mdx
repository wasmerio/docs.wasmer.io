...
To get started, install both the Wasmer runtime and the wasmer-cli:
Todo: say that we need the wasmer-cli, but how to download the latest version?
...
You can use `wasmer --version` to verify that the installation was successful.
...
```shell copy
wasmer login --registry https://registry.wasmer.io <token>
```
