...
[Sign in to PlanetScale](https://auth.planetscale.com/sign-in), and create an account if needed. Note that free accounts are no longer available. Choose the [plan](https://planetscale.com/pricing) that fits your needs.
...
Use any standard MySQL client, such as the `mysql` CLI. For this guide, we'll use the MySQL option.
