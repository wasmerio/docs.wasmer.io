...
You will not notice these failures directly, because Edge uses a fresh regular
instance of your app to serve requests if no snapshot is available, and builds
the snapshot in the background.

To identify issues, you can enable the app debug mode for your app.

Simply add the following to your `app.yaml` and re-deploy:
```yaml
debug: true
```

In debug mode the InstaBoot snapshot will not be built in the background,
but directly during regular request processing.
If a warm-up request fails, the request will fail and you will get an informative
error message.
