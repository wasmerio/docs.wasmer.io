...
- `name` \(string\): the package name, as `<namespace>/<name>`. Only required when publishing a named package to the registry; unnamed packages can be published without it.
- `version` \([semver version](https://semver.org/)\): a valid Semantic Version. Only required when publishing a named package to the registry.
- `description` \(string\)
- `private` \(bool\): mark the package as private. Defaults to `false`.
...
Dependencies can provide modules for your own commands. To use a module from a
...
main-args = ["-c", "print('Hello from a dependency module')"]
```
...
- `abi` \(string\): the ABI the module targets. One of `"wasi"`, `"wasm4"`, or `"none"`.
...
Bindings come in two forms, [WAI][wai] or [WIT](https://component-model.bytecodealliance.org/design/wit.html),
and exactly one of `wai-version` or `wit-bindgen` must be set.

WAI form:

- `wai-version` \(string\): The version of [WAI][wai] being targeted (e.g. `"0.2.0"`)
...
- `wit-bindgen` \(string\): The version of `wit-bindgen` being targeted (e.g. `"0.1.0"`)
- `wit-exports` \(string\): The path to a `*.wit` file defining the interface exposed by the module

```toml filename="wasmer.toml" copy
[module.bindings]
wit-bindgen = "0.1.0"
wit-exports = "./exports.wit"
```
...
- `runner` **required** \(string\): a URL or well-known name for the runner used to execute this command (e.g. `"wasi"`, `"wcgi"`, `"wasm4"`)
