...
In [the previous tutorial][run], we created a basic Markdown editor by wiring
a `<textarea>` up to a Markdown renderer compiled to WASI. Now, we're going to
take things the next level by converting the renderer to a package and running
it from [the Wasmer registry][registry].
...
```sh
$ ls
Cargo.lock  Cargo.toml  src  target  wasmer.toml
```
