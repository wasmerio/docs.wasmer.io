...
#### Configure compiler thread usage

For the Cranelift and LLVM backends, you can control the number of threads used for compilation:

```bash copy
wasmer run my_program.wasm --cranelift --cranelift-num-threads=4
```

This setting can improve compilation performance on multi-core systems by parallelizing the compilation process.
