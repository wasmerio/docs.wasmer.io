...
```shell
wasmer app database list [APP_SPECIFIER]
```

Where `APP_SPECIFIER` is optional and can be in the format `owner/app-name` or the app ID. If omitted, the command will use the app in the current directory.

The command will display a table with information about each database, including:
- Database name
- Host address
- Port number
- Database type
...
→ Name: my-postgres-db
→ Host: postgres-xxxxx.wasmer.app
→ Port: 5432
→ Type: PostgreSQL
→ UI: https://wasmer.io/apps/username/app-name/databases/my-postgres-db
```

By default, the command doesn't show sensitive information. If you need to see database credentials including passwords, you can use the `--with-password` flag:

```shell
wasmer app database list --with-password
```

With this flag, the output will include additional credential information:
```
  Database Information  
→ Name: my-postgres-db
→ Host: postgres-xxxxx.wasmer.app
