...
You can restrict your apps to only run in specified regions with the
[`locality.regions` setting](/edge/configuration#localityregions) in `app.yaml`.
...
If you need more control, one of the features on our roadmap is persistent instances, which will keep running, and won't be dynamically spawned or terminated.

A note about database locality:
The [`locality.regions` setting](/edge/configuration#localityregions) allows you to restrict your app to certain regions, so you can keep your instances close to the location of your database.
