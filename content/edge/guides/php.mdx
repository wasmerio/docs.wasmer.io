...
In this guide, you'll learn the process of deploying a PHP application on Wasmer Edge.
...
You can set [`scaling.mode`](/edge/configuration#scalingmode) in your `app.yaml` file:
...
You can customize the PHP settings with a custom `php.ini`. This requires
adding your own `wasmer.toml` manifest to the project, which replaces the
default package configuration generated for the template.
...
Now, in your `wasmer.toml`, map the `config` directory into the filesystem (`fs`) and set the environment variable `PHPRC`
