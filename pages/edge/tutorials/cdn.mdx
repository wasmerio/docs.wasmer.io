...
Let's first delete the `public` directory.
...
So, let's add a `config.toml` file in the `app` directory.
...
main-args = ["-w", "config.toml"]
```

### Adding files for hosting

Now, let's add a `public` directory in our `app` and some files to the `public` directory.
...
And now our `index.html` file looks like this:
...
You can view our app [here](https://wasmer-edge-as-cdn.wasmer.app).
