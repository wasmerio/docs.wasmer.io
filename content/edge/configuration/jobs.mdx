...
- **trigger** *(str, required)* – Defines when the job should run.
  - Use a duration such as `15m`, `1h`, or `24h` for simple interval
    schedules.
  - Use a cron expression for uniform schedules, such as
   ...
   action:
     fetch:
       ...
       headers:
         ...
         User-Agent: Wasmer-CronJob
       expect:
         status_codes: [200, 204]
   ```
   - `path` *(str, Optional)* – The request URL path. Defaults to `/`.
   - `method` *(str, Optional)* – HTTP method. Defaults to `GET`.
   - `headers` *(map[str, str], Optional)* – HTTP headers keyed by header
     name. Only string values are sent.
...
This job runs `php --version` every 15 minutes.
...
This job runs before a new app version is created.
...
This job fetches the `/warm-up-caches` path on its app after
each deployment.
...
- **Pre-deployment migrations:** Run setup or migration commands before
