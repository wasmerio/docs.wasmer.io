import { Card, Callout, Steps, FileTree } from "@components/nextra-theme";
...
First, remove the `markdownRendererUrl` import. We will load the published
package from the registry instead of fetching a local `*.wasm` file.

```diff filename="index.ts"
 import { init, Wasmer, Command } from "@wasmer/sdk";
-import markdownRendererUrl from "./markdown-renderer/target/wasm32-wasip1/release/markdown-renderer.wasm?url";
```

Next, instead of fetching the local `*.wasm` file and wrapping it with
`Wasmer.fromWasm()`, we use `Wasmer.fromRegistry("...")` to load a package from
the registry.

```diff filename="index.ts"
 async function initialize() {
     await init();
-    const response = await fetch(markdownRendererUrl);
...
The `renderMarkdown()` function already accepts a `Command`, so it does not need
to change. A `Command` is an executable element inside a Wasmer package and will
use [the WASIX runner][wasix-runner] under the hood.

Finally, we update `main()` to rename `command` to `pkg` and pass
...
-        const renderedHtml = await renderMarkdown(command, markdownInput.value);
...
means the package name in `Wasmer.fromRegistry()` does not match a package that
has been published to the registry:
...
Use the package name you published in `wasmer.toml`. If the console mentions
`SharedArrayBuffer` or `crossOriginIsolated`, make sure the `vite` dev server
sets the `Cross-Origin-Opener-Policy` and `Cross-Origin-Embedder-Policy`
headers. See [Configure the Dev Server][dev] for more.
