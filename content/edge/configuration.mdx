...
Enable email sending support for the app.
...
See [Sending Email](/edge/learn/email) for setup details and examples.
...
Auto-provision a managed database for the app.
...
If `engine` is omitted, the database is MySQL. The engine of an existing
database cannot be changed; delete the database first. An app can have one
database.

**Note**: databases are tied to regions. Specifying a region with the
`locality.regions` setting is optional; if omitted, the database is created in
the default database region. Specifying multiple regions is an error.
...
See the [Databases documentation](/edge/learn/databases) for connection
examples, credential rotation, backups, and troubleshooting.
